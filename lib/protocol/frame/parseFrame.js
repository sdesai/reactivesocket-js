...
function parseSetupFrame(buffer) {
    ...
    var metadataMime = buffer. slice(mimeOffset, mimeOffset + metadataMimeLength). toString();

    mimeOffset += metadataMimeLength;

    var dataMimeLength = buffer.readUInt8(mimeOffset++);
    var dataMime = buffer. slice(mimeOffset, mimeOffset + metadataMimeLength). toString();
