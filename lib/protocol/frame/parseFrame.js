...
/**
 * @param {Buffer} buf -
 * @param {String} [encoding] - The encoding of the frame. Since the encoding
 * is specified in the setup frame, we must pass it in after the setup frame
 * has been parsed.
...
function parseSetupFrame(buffer) {
    ...
    var metadataMime = buffer. slice(mimeOffset, mimeOffset +
                                     metadataMimeLength). toString();

    mimeOffset += metadataMimeLength;

    var dataMimeLength = buffer.readUInt8(mimeOffset++);
    var dataMime = buffer.slice(mimeOffset, mimeOffset + metadataMimeLength)
        .toString();
