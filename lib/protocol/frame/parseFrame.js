 ...
 */
function parse(b, lengthOptional) {
    assert.object(b, 'buffer');
    assert.optionalBool(lengthOptional, 'lengthOptional');

    LOG.debug({buffer: b}, 'parse: entering');
    ...
    switch (frame.header.type) {
        case TYPES.ERROR:
            parseErrorCode(buf, frame);
            parseMetadata(buf, frame);
            parseData(buf, frame);
            break;
        case TYPES.SETUP:
            parseSetupFrame(buf, frame);
            parseMetadata(buf, frame);
            parseData(buf, frame);
            break;
        case TYPES.RESPONSE:
    ...
    // delete _offset on the way out.
    delete frame._offset;
...
function parseMetadata(b, frame) {
    ...
    var buffer = b.buffer;
    var md;
    var hasMetadata = metadata.hasMetadata(frame.header.flags);

    if (hasMetadata) {
        var mdLength = buffer.readUInt32BE(b.offset);
        b.offset += META_DATA_LENGTH;
        md = buffer.slice(b.offset, b.offset + mdLength);
        b.offset += mdLength;
        frame.metadata = md;
    }
...
function parseData(b, frame) {
    ...
    var buffer = b.buffer;
    frame.data = buffer.slice(b.offset);
    LOG.debug({data: frame.data, offset: b.offest}, 'parsed data');

    // data is the last part of the frame, so off set is just buf len
