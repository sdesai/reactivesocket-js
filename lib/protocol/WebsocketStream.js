...
function WebsocketStream(opts, cb) {
    ...
    self._websocket.on('open', function openWS() {
        self._log.info('websocket connection established');
        self.emit('connect');
        return cb();
    });

    self._websocket.on('message', function messageWS(data, flags) {
        self._log.debug({data: data, flags: flags}, 'got message from ws');
        self.push(data);
    });

    self._websocket.once('close', function closeWS(code, message) {
        ...
        self._buffers.forEach(function forEach(buf) {
            self.push(buf);
        });
        self.push(null);
    });

    self._websocket.on('error', function errorWS(err) {
...
WebsocketStream.prototype._write = function _write(chunk, encoding, cb) {
    ...
    self._websocket.send(chunk, {binary: true}, function _writeSend(err) {
