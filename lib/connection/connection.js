...
Connection.prototype._setup = function _setup(frame) {
    ...
    // decode the metadata and data
    var payload = self._decodePayload(frame);
...
Connection.prototype._request = function _request(frame) {
    ...
    if (stream.request.data) {
        stream.request.data += payload.data;
    } else {
        stream.request.data = payload.data;
    }

    if (stream.request.metadata) {
        stream.request.metadata += payload.metadata;
    } else {
        stream.request.metadata = payload.metadata;
    }
...
Connection.prototype._response = function _response(frame) {
    ...
    if (stream.response.data) {
        stream.response.data += payload.data;
    } else {
        stream.response.data = payload.data;
    }

    if (stream.response.metadata) {
        stream.response.metadata += payload.metadata;
    } else {
        stream.response.metadata = payload.metadata;
    }
...
Connection.prototype._decodePayload = function _decodePayload(frame) {
    var self = this;
    var payload = {};
    if (frame.data) {
