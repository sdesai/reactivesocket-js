...
module.exports = function compareFrames(expected, actual) {
    ...
    expect(actualLength, EXPECT_STRING + 'LENGTH (0 - 4)'). to.equals(expectedLength);

    // then the type
    var expectedType = friendlyHex(expected.slice(4, 6));
    var actualType = friendlyHex(actual.slice(4, 6));
    expect(actualType, EXPECT_STRING + 'TYPE (4 - 6)'). to.equals(expectedType);

    // then the flags
    var expectedFlags = friendlyHex(expected.slice(6, 8));
    var actualFlags = friendlyHex(actual.slice(6, 8));
    expect(actualFlags, EXPECT_STRING + 'FLAGS (6 - 8)'). to.equals(expectedFlags);
    ...
    expect(actualRestOfData, EXPECT_STRING + 'REST OF DATA (8+)'). to.equals(expectedRestOfData);
};
